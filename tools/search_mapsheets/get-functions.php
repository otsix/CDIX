...
//		$sqlquery = "SELECT *,  st_asgeojson(the_geom) as geom from incidencias where id_issue=" . $id;
		$sqlquery = "SELECT *,  st_asgeojson(the_geom,0,0) as geom from malla5_etrs_concellos where clasico= . $id";
...
//problemas con conn	
		...
		//if($DEBUG_MODE){debug($count_bbox);}
	...
	public function getGeometryJSON($tbl, $col, $id, $isString){
		global $DEBUG_MODE;
		$sqlbbox = 'SELECT ST_AsGeoJSON(the_geom,0,0) as the_geom ' .
		'FROM '.$tbl . ' ';
		if ($isString) {
			$sqlbbox .= ' WHERE "'.$col."\"='" . $id . "';";
		} else {
