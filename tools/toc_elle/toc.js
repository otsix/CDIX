...
TOC = function(map) {
	...
	this.initTOC = function(base_layers) {
		...
		//GET AccordionGroups from DB (_map and _wms_layers)
		//Here we ask for the groups of the layers on the ToC
		$.ajax({
			type : "GET",
			// Wait response for creation groups
			async: false,
			url : "tools/toc_elle/get-elle-layers.php",
			data : "getgroups=1",
			dataType : "json",
			success : this.createGroups,
			context : this,
			error : function(response) {
				alert("Error na resposta Groups.");
			}
		});
		//Here we ask for the layers on the ToC
		//TODO: ELLE is not used any more
		// $.ajax({
			// url : "tools/toc_elle/get-elle-layers.php",
			// type : "GET",
			// dataType : "json",
			// success : this.loadELLELayers,
			// context : this,
			// error : function(response) {
				// alert("Error na resposta ELLE.");
			// }
		// });
		...
		$.ajax({
			...
			async : false,
			url : "tools/toc_elle/get-elle-layers.php",
			data : "layers=_wms_layers",
			dataType : "json",
			success : this.loadWMSLayers,
			context : this,
			error : function(response) {
				alert("Error na resposta WMS_Layers.");
			}
		});

		// Read all layers from layers.js file		
		for(var n in base_layers) {
			var new_layer = base_layers[n];
			this.addLayerOnTOC('Capas Base', new_layer, new_layer.name, 'base');
		}
	 ...
	 */
	this.getLayer = function(layerID, layerTitle, visible, isBase, params) {
		var default_params = {
			isBaseLayer : isBase,
			singleTile : true,
			visibility : visible,
		...
		// BE CAREFUL
		//     "params" column on the database must use double quotes on eache key and value
		//     but not on the before and after the main brackets 
		params = $.extend(default_params,$.parseJSON(params));
	...
	this.getWMSLayer = function(layerURL, layerID, layerTitle, visible, isBase, params) {
		var lyr = this.getLayer(layerID, layerTitle, visible, isBase, params);
		lyr.url = layerURL;
		return lyr;
	}
	 ...
	 * {String} layer title
	 */
	this.addLayerOnTOC = function(group, new_layer, title, db_tablename) {
		var idGroup = $(".capas .capa-section1").children().length;
		idGroup = group.split(' ').join('').toLowerCase();
		...
		if(new_layer.displayInLayerSwitcher) {
			...
			inputElem.name = (baseLayer) ? idGroup + "_baseLayers" : new_layer.name;
			...
			$(labelSpan).attr("db_tablename", db_tablename);
			...
			$(".capas .layer_group" + idGroup).append(line);
	...
	this.groupExists = function(group_name) {
		//Check if group_name already exists
		span_acc_groups = $(".capas .capa-section1 a div").children();
		for (var i = 0; i < span_acc_groups.length; i++){
			span_acc_group = span_acc_groups[i];
			acc_group_name = $(span_acc_group).text();
			if (acc_group_name == group_name){
				// "group_name" already created on the ToC
				return true;
			}
		}
		return false;
	...
	this.createGroups = function(group_list) {
		for (var i = 0; i < group_list.length; i++){
			var group = group_list[i];
			this.createNewGroupOnAccordion(group.group)
		}
	}
	...
	this.createNewGroupOnAccordion = function(group_name) {
		...
		//console.log("Lets create group: " + group_name);	
		var idGroup = group_name.split(' ').join('').toLowerCase();
		...
		'        </div></a>' + '<ul class="form-section' + despleg_num + ' layer_group' + idGroup + 
		'" style="margin-left: 10px;">' + '</ul></li>';

		$(".capas .capa-section1").append(html_code_new_group);
		set_expand_collapse_click($(".capas .capa-section1").children().last()); 
	};

	// /*
	 // * Method: loadELLELayers
	 // * Parameters:
	 // * {Object JSON}
	 // * Receives response from layers service with JSON
	 // * structure
	 // */
	// this.loadELLELayers = function(elle_toc) {
// 
		...
		// for(var i = lyrs_num - 1; i >= 0; i--) {
			...
			// }
			// var layer_id = elle_toc[i].nombre_tabla;
			// var title = elle_toc[i].nombre_capa;
			// var isVisible = (elle_toc[i].visible == "f") ? false : true;
			// var new_layer = this.getLayer(layer_id, title, isVisible, false, '');
			// new_layer.group = group_name;
			// new_layer.db_tablename = layer_id;
			// this.addLayerOnTOC(elle_toc[i].grupo, new_layer, title, layer_id);
		// }
...
// 
	...
	/*
	 * Method: loadWMSLayers
	 * Parameters:
	 * {Object JSON}
	 * Receives response from layers service with JSON
	 * structure (similar to loadELLELayers)
	 */
	this.loadWMSLayers = function(wms_toc) {
		var lyrs_num = wms_toc.length;
		var group_name = '';
		for(var i = lyrs_num - 1; i >= 0; i--) {
			var create_new_group = false;
			if(group_name == '' || wms_toc[i].layer_group != group_name) {
				group_name = wms_toc[i].layer_group;
				create_new_group = true;
			}
			if(!wms_toc[i].layer_group) {
				//if no group, then default
				group_name = 'default';
				create_new_group = true;
			}
			...
			var layer = wms_toc[i].wms_layer;
			var title = wms_toc[i].layer_name;
			var urlLayer = wms_toc[i].wms_url;
			var isBase = (wms_toc[i].is_base == "f") ? false : true;
			var isVisible = (wms_toc[i].visible == "f") ? false : true;
			var new_layer = this.getWMSLayer(urlLayer, layer, title, isVisible, isBase, wms_toc[i].parameters);
			new_layer.group = group_name;
			this.addLayerOnTOC(group_name, new_layer, title, 'none');
		}
		//expand_collapse_accordions();
...
Selection = function() {
	...
	this.launch = function(evt) {
		...
		if(layerLine.length == 0) {
			...
			if(!evt.data.layer.isBaseLayer) {
				// ONLY FOR CDIX COMMENT next 2 lines:
